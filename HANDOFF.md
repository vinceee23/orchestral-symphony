...
- **➡️ NEXT ACTION = the final-resim phase (pre-merge), 3 open items:**
  - **(A) era-pacing FAILS — needs Vince's design call.** At `MAX_STEPS=250k` it's step-capped at 433k/1M records (Platinum not reached). Raising to 1M steps DOES reach Platinum — but **at 159.8h on `opusCount=1`** with >25-min dead-zones, so it then fails the dead-zone assert. The era sim does **one** Magnum Opus then grinds records linearly; the realistic `l3-pacing` sim reaches Platinum ~22h. **Question for Vince:** is the Platinum path supposed to be *repeated-MO-driven* (so the era sim's 1-MO model is unfaithful and should do more MOs), or is there a real 100h+ records dead-grind to fix? This predates the reconcile. Don't budget-bump alone — it just surfaces the dead-zone.
